...
const LogViewerSearch = (_a) => {
    ...
    const [indexAdjuster, setIndexAdjuster] = react_1.useState(0);
    const { searchedWordIndexes, scrollToRow, setSearchedInput, setCurrentSearchedItemCount, setRowInFocus, setSearchedWordIndexes, currentSearchedItemCount, searchedInput, itemCount } = react_1.useContext(LogViewerContext_1.LogViewerToolbarContext);
    const { parsedData } = react_1.useContext(LogViewerContext_1.LogViewerContext);
    ...
    react_1.useEffect(() => {
        ...
        const adjustedSearchedInput = utils_1.escapeString(searchedInput);
        if (adjustedSearchedInput !== '' && adjustedSearchedInput.length >= minSearchChars) {
            foundKeywordIndexes = utils_1.searchForKeyword(adjustedSearchedInput, parsedData, itemCount || parsedData.length);
    ...
    return (react_1.default.createElement(react_core_1.SearchInput, Object.assign({ placeholder: placeholder, value: searchedInput, resultsCount: `${currentSearchedItemCount + indexAdjuster} / ${hasFoundResults ? searchedWordIndexes.length : 0}` }, props, { onChange: (input, event) => {
