...
exports.LogViewerRow = react_1.memo(({ index, style, data, ansiUp }) => {
    const { parsedData, searchedWordIndexes, rowInFocus } = data;
    const context = react_1.useContext(LogViewerContext_1.LogViewerContext);
    ...
    const getFormattedData = () => {
        ...
        if (context.searchedInput) {
            const splitAnsiString = utils_1.splitAnsi(rowText);
            const regEx = new RegExp(`(${utils_1.escapeString(context.searchedInput)})`, 'ig');
            const composedString = [];
            splitAnsiString.forEach(str => {
                matchCounter = 0;
                if (utils_1.isAnsi(str)) {
                    ...
                    splitString.forEach((substr, newIndex) => {
                        if (substr.match(regEx)) {
                            matchCounter += 1;
                            composedString.push(server_1.default.renderToString(react_1.default.createElement("span", { className: react_styles_1.css(log_viewer_1.default.logViewerString, handleHighlight(matchCounter)), key: newIndex }, substr)));
                        }
                        else {
                            composedString.push(utils_1.escapeTextForHtml(substr));
                        }
                    });
                }
            });
        ...
        return utils_1.escapeTextForHtml(rowText);
    };
    return (react_1.default.createElement("div", { style: style, className: react_styles_1.css(log_viewer_1.default.logViewerListItem) },
        react_1.default.createElement("span", { className: react_styles_1.css(log_viewer_1.default.logViewerIndex) }, getRowIndex(index)),
        react_1.default.createElement("span", { className: react_styles_1.css(log_viewer_1.default.logViewerText), style: { width: 'fit-content' }, dangerouslySetInnerHTML: { __html: ansiUp.ansi_to_html(getFormattedData()) } })));
});
